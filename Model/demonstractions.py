    ...
    with open(train_file_path, 'r', encoding='utf-8') as f:
        for line in f:
            ...
            if data["input"].startswith("Please list all scientific entities of type"):
                # 剔除"Entities of type [] may exist in text"
...
def compute_edit_distance(test_text, candidate_texts):
    # 编辑距离在CPU上计算
    edit_distances = [Levenshtein.distance(test_text, cand) for cand in candidate_texts]
    return np.array(edit_distances)


def select_demonstrations(test_text, E_candidate, model, k=5, alpha=0.4, beta=0.3, gamma=0.2, delta=0.1):
    # 将文本转为向量，放在GPU上
    test_embedding = model.encode([test_text], convert_to_tensor=True, device="cuda")
    candidate_embeddings = model.encode(E_candidate, convert_to_tensor=True, device="cuda")

    cosine_similarities = cosine_similarity(test_embedding.cpu().numpy(), candidate_embeddings.cpu().numpy()).flatten()
    edit_distances = compute_edit_distance(test_text, E_candidate)
    ...
    edit_scores_norm = (edit_distances - np.min(edit_distances)) / (
            np.max(edit_distances) - np.min(edit_distances))

    final_scores = alpha * sim_scores_norm + beta * div_scores_norm + gamma * (1 - div_scores_norm) + delta * (
                1 - edit_scores_norm)
